...
class TrainSerializer(ModelSerializer):
    ...
    class Meta:
        model = trainDetailModel
        fields = ["train_number", "train_name", "source", "destination", "departure", "arrival", "departuretime", "arrivaltime", "duration", "price", "available_seats", "tax", "total", "departureDate", "arrivalDate"]
...
class BookingSerializer(ModelSerializer):
    ...
    def validate_no_of_seats(self, value):
        ...
        """
        id = self.initial_data.get('trainPrice')
        try:
            train_instance = trainPriceModel.objects.get(id = id)
        except ObjectDoesNotExist:
            return Response({'status':'Failure', 'message':"train no longer exists"}, status=status.HTTP_404_NOT_FOUND)
        else:
            available_seats = train_instance.available_seats
    ...
    def get_price(self, obj):
        ...
        serialized_data = TrainSerializer(obj.train).data
