...
class FlightSerializer(ModelSerializer):
    ...
    class Meta:
        model = flightDetailModel
        fields = ["flight_number", "airline", "source", "destination", "departure", "arrival", "departuretime", "arrivaltime", "duration", "price", "available_seats", "tax", "total", "departureDate", "arrivalDate"]
...
class BookingSerializer(ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        booking = BookingsModel.objects.create(**validated_data)
        
        for passenger in passengerDetails:
            passenger.update({'booking':booking.id})
            data = PassengerSerializer(data = passenger)
    ...
    def validate_no_of_seats(self, value):
        ...
        """
        id = self.initial_data.get('flightPrice')
        try:
            flight_instance = flightPriceModel.objects.get(id = id)
        except ObjectDoesNotExist:
            return Response({'status':'Failure', 'message':"flight no longer exists"}, status=status.HTTP_404_NOT_FOUND)
        else:
            available_seats = flight_instance.available_seats
    ...
    def get_price(self, obj):
        ...
        serialized_data = FlightSerializer(obj.flight).data
